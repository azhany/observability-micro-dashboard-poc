...
## Tasks / Subtasks
- [ ] Task 1: Update Dashboard Controller (AC: 1, 2)
    - [ ] Modify `app/app/Http/Controllers/DashboardController.php` to calculate `last_seen` and `status` for each agent.
    - [ ] Use `DB::raw('MAX(timestamp)')` to find the last reported metric for each `agent_id`.
    - [ ] Implement status logic: "Online" if `last_seen` is within the last 60 seconds, otherwise "Offline".
- [ ] Task 2: Enhance Realtime Chart Component (AC: 4, 5)
    - [ ] Update `app/resources/js/Components/RealtimeChart.vue` to support an optional `agentId` prop.
    - [ ] Update `handleData` logic to:
...
- [ ] Task 3: Update Dashboard Overview Page (AC: 1, 3)
    - [ ] Update `app/resources/js/Pages/Dashboard/Overview.vue` to display the `last_seen` timestamp for each agent.
    - [ ] Ensure the status badge reflects the "Online"/"Offline" status correctly (e.g., gray for offline).
- [ ] Task 4: Integrate Charts into Tenant Detail Page (AC: 3, 4, 5)
    - [ ] Update `app/resources/js/Pages/Dashboard/TenantDetail.vue` to import and use `RealtimeChart.vue`.
    - [ ] Replace placeholders for CPU, Memory, and Disk with `RealtimeChart` instances.
    - [ ] Pass the `agent_id` filter (from query params) to the `RealtimeChart` components.
    - [ ] Configure charts for the specific Go agent metrics: `cpu.usage.percent`, `memory.used.bytes`, `disk.usage.percent`.
