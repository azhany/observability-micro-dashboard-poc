...
| 2026-01-20 | 1.1 | Applied QA fixes: agent_id filtering + adaptive alert limits | James (Dev Agent) |
...
**QA Fixes (v1.1):**
- **HIGH PRIORITY**: Added agent_id to MetricController API response and query parameters
  - Previously: agent_id only available in live SSE mode, historical queries lacked agent filtering
  - Fixed: MetricController now includes agent_id in SELECT and supports filtering via query parameter
  - Impact: Eliminates data mixing across agents in historical views
- **HIGH PRIORITY**: Removed ineffective client-side agent filtering from MetricService
  - Previously: MetricService attempted client-side filtering which masked the API issue
  - Fixed: Now passes agent_id directly to API for server-side filtering
- **MEDIUM PRIORITY**: Implemented adaptive limit for AlertController based on query type
  - Previously: Hard limit of 100 alerts could truncate 7-day historical views
  - Fixed: Limit=100 for ad-hoc queries, limit=1000 for time-ranged queries
  - Added: Response now includes "limit" and "has_more" indicators for client awareness
- Added 3 new tests: agent_id filtering, alert limit behavior, truncation indicator
- All tests passing: 103/103 PHP tests (3 new), 52/52 frontend unit tests
...
**Frontend Services:**
- app/resources/js/Services/MetricService.ts (created, modified in v1.1 for QA fixes)

**Backend Controllers:**
- app/app/Http/Controllers/Api/AlertController.php (created, modified in v1.1 for QA fixes)
- app/app/Http/Controllers/Api/MetricController.php (modified in v1.1 for QA fixes)
...
- tests/Feature/AlertQueryApiTest.php (created, modified in v1.1 with 2 new tests)
- tests/Feature/MetricQueryApiTest.php (modified in v1.1 with 1 new test)
...
### Review Date: 2026-01-20 (Re-Review v1.1)

### Reviewed By: Quinn (Test Architect)

### Code Quality Assessment

The developer has successfully addressed the primary concerns from the previous review. The API now correctly supports server-side agent filtering, and the alert limits are adaptive.

**Improvements Verified:**
- **Agent Filtering**: `MetricController` now returns `agent_id` and filters by it. `MetricService` uses server-side filtering. This fixes the data contamination issue.
- **Alert Limits**: `AlertController` now uses a limit of 1000 for time-ranged queries, significantly reducing the risk of truncation for the 7-day view.
- **Testing**: New tests confirm the fix for agent filtering and limit logic.

### Compliance Check

- All ACs Met: [✓] All original ACs met, plus stability improvements.

### Improvements Checklist

- [x] **High Priority**: Investigate `MetricController` response. It likely needs to include `agent_id` to support proper historical filtering per agent.
- [x] **Medium Priority**: Increase `AlertController` limit or implement pagination/time-windowing that guarantees coverage of the requested window.
- [ ] **Low Priority**: Optimize `chartjs-plugin-annotation` usage. (Still valid for future optimization)

### Gate Status

Gate: **PASS** → docs/qa/gates/4.4-historical-trend-charting.yml
