...
The implementation is solid. The state machine logic correctly handles the `OK -> PENDING -> FIRING` transitions and their reverses. The database schema supports the requirements well. Code is clean and readable.

### Refactoring Performed

- **File**: `app/app/Jobs/EvaluateAlertRules.php`
  - **Change**: Changed `AlertRule::all()` to `AlertRule::cursor()`.
  - **Why**: `all()` loads every rule into memory simultaneously.
  - **How**: `cursor()` uses a database cursor to stream records, significantly reducing memory usage when the number of rules grows.

### Compliance Check

- Coding Standards: [✓]
- Project Structure: [✓]
- Testing Strategy: [✓] High coverage of state transitions.
- All ACs Met: [✓]

### Improvements Checklist

- [x] Refactored `EvaluateAlertRules` to use `cursor()` for memory efficiency.
- [ ] Future: Optimize the N+1 query pattern in `EvaluateAlertRules` (currently queries metrics for every rule individually).

### Security Review

Tenant isolation is correctly implemented in queries and verified by `test_alerts_are_isolated_by_tenant`.

### Performance Considerations

The current implementation queries the `metrics` table once per rule. For a high number of rules (e.g., >1000), this will become a bottleneck. It is acceptable for the POC but is a primary candidate for optimization (e.g., batching or inverted indexing) in v2.
...
Gate: PASS → docs/qa/gates/4.2.stateful-alerting-engine-rule-evaluator.yml
